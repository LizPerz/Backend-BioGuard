...
public class MLServiceTests
{
    ...
    [Fact]
    public async Task DesactivarModeloAsync_ModeloExiste_Desactiva()
    {
        var mockResult = new Mock<UpdateResult>();
        mockResult.Setup(r => r.ModifiedCount).Returns(1);

        _mockModelos.Setup(c => c.UpdateOneAsync(
            It.IsAny<FilterDefinition<ModeloMl>>(),
            It.IsAny<UpdateDefinition<ModeloMl>>(),
            It.IsAny<UpdateOptions>(),
            It.IsAny<CancellationToken>()))
            .ReturnsAsync(mockResult.Object);

        await _service.DesactivarModeloAsync("m1");

        _mockModelos.Verify(c => c.UpdateOneAsync(
            It.IsAny<FilterDefinition<ModeloMl>>(),
            It.IsAny<UpdateDefinition<ModeloMl>>(),
            It.IsAny<UpdateOptions>(),
