...
public class UsuariosWebServiceTests
{
    ...
    public UsuariosWebServiceTests()
    {
        ...
        mockImageStorage.Setup(s => s.UploadAsync(It.IsAny<string>(), It.IsAny<string?>()))
            .ReturnsAsync(new ImageUploadResult(true, "https://img.url", null));
        var mockEmailService = new Mock<IEmailService>();
        _service = new UsuariosWebService(_mockDb.Object, mockLogger.Object, mockImageStorage.Object, mockEmailService.Object);
    }
    ...
    {
        var currentUser = new UsuarioWeb
        {
            Id = "user123",
            Correo = "actual@test.com",
            PasswordHash = PasswordHasher.Hash("MiPassActual123!")
        };

        _mockDb.SetupSequence(db => db.FindFirstOrDefaultAsync(
                It.IsAny<IMongoCollection<UsuarioWeb>>(),
                It.IsAny<System.Linq.Expressions.Expression<Func<UsuarioWeb, bool>>>()))
            .ReturnsAsync(currentUser)       // user lookup
            .ReturnsAsync((UsuarioWeb?)null); // email check (free)
        ...
        var result = await _service.CambiarCorreoAsync("user123", "nuevo@test.com", "MiPassActual123!");
    ...
    public async Task CambiarCorreoAsync_CorreoYaExiste_RetornaFalse()
    {
        ...
        _mockDb.SetupSequence(db => db.FindFirstOrDefaultAsync(
                ...
                It.IsAny<System.Linq.Expressions.Expression<Func<UsuarioWeb, bool>>>()))
            .ReturnsAsync(currentUser)  // user lookup
            .ReturnsAsync(existente);    // email check (taken)
    ...
    public async Task SubirFotoAsync_DatosValidos_RetornaTrue()
    {
        ...
        var (success, url) = await _service.SubirFotoAsync("user123", "base64data");
