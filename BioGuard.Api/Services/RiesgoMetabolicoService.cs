...
public class RiesgoMetabolicoService : IRiesgoMetabolicoService
{
    ...
    public async Task UpdateBaselineAsync(string pacienteId, LecturaSensor lectura)
    {
        var baseline = await GetOrCreateBaselineAsync(pacienteId);
        if (baseline.TotalLecturas % 50 == 0)
        {
            await GetOrCreateBaselineAsync(pacienteId);
        }
    }
    ...
    public Task<AlertTrigger?> CheckAlertTriggerAsync(string pacienteId, IrmeResult irmeResult)
    {
        ...
        {
            return Task.FromResult<AlertTrigger?>(new AlertTrigger(
