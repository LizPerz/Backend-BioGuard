...
public class PacientesIntegrationTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task UpdateBiometria_DatosValidos_Retorna200()
    {
        ...
        var request = new UpdateBiometriaRequest(new DateTime(DateTime.Today.Year - 30, 1, 1), "M", 75.5, 175.0, false, false, "Moderada");
    ...
    public async Task RegenerarQR_PacienteExiste_Retorna200()
    {
        ...
        var mockUpdateResultPaciente = new Mock<UpdateResult>();
        mockUpdateResultPaciente.Setup(r => r.ModifiedCount).Returns(1);
        _mockPacientes.Setup(c => c.UpdateOneAsync(
            It.IsAny<FilterDefinition<Paciente>>(),
            It.IsAny<UpdateDefinition<Paciente>>(),
            It.IsAny<UpdateOptions>(),
            It.IsAny<CancellationToken>()))
            .ReturnsAsync(mockUpdateResultPaciente.Object);
    ...
    [Fact]
    public async Task ObtenerDashboardSummary_ConTokenValido_RetornaOk()
    {
        var pacienteId = "123456789012345678901234";
        ...
        var paciente = new Paciente
        {
            ...
            UsuarioWebId = "user123",
            Nombre = "Juan",
            Biometria = new Biometria { EsDiabetico = true },
            PerfilCompletado = true
        };

        _mockDb.Setup(db => db.FindFirstOrDefaultAsync(
                _mockPacientes.Object,
                It.IsAny<System.Linq.Expressions.Expression<Func<Paciente, bool>>>()))
            .ReturnsAsync(paciente);
        ...
        _mockDb.Setup(db => db.FindToListAsync(
            ...
            {
                new() { Id = "l1", PulsoBpm = 80, TemperaturaC = 36.5, SudoracionGsr = 2.0, NivelRiesgo = "Leve", Timestamp = DateTime.UtcNow }
            });
        ...
        _mockDb.Setup(db => db.FindFirstOrDefaultAsync(
            ...
            .ReturnsAsync(new TrackingGps { Id = "t1", EsEmergencia = false, Timestamp = DateTime.UtcNow, Ubicacion = new UbicacionGps { Coordinates = new[] { -99.1, 19.4 } } });

        _mockDb.Setup(db => db.FindFirstOrDefaultAsync(
                _mockDispositivos.Object,
                It.IsAny<System.Linq.Expressions.Expression<Func<Dispositivo, bool>>>()))
            .ReturnsAsync(new Dispositivo { Id = "d1", NombreDispositivo = "WearOS Device", MacAddress = "AA:BB:CC:DD:EE:FF", Conectado = true });
        ...
        var response = await _client.GetAsync($"/api/Pacientes/{pacienteId}/dashboard-summary");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await response.Content.ReadAsStringAsync();
        var doc = JsonDocument.Parse(json);
        doc.RootElement.GetProperty("paciente").GetProperty("nombre").GetString().Should().Be("Juan");
        doc.RootElement.GetProperty("dispositivo").GetProperty("nombreDispositivo").GetString().Should().Be("WearOS Device");
    }
