...
public class AuditoriaServiceTests
{
    ...
    [Fact]
    public async Task ObtenerAsync_ConEntidadId_FiltraPorEntidad()
    {
        var registros = new List<Auditoria>
        {
            new() { Id = "aud1", Accion = "Login", EntidadId = "user1", Fecha = DateTime.UtcNow }
        };

        FilterDefinition<Auditoria>? capturedFilter = null;
        _mockDb.Setup(db => db.FindToListAsync(
                It.IsAny<IMongoCollection<Auditoria>>(),
                It.IsAny<FilterDefinition<Auditoria>>(),
                It.IsAny<SortDefinition<Auditoria>>(),
                It.IsAny<int?>(),
                It.IsAny<int?>()))
            .Callback<IMongoCollection<Auditoria>, FilterDefinition<Auditoria>, SortDefinition<Auditoria>, int?, int?>(
                (_, filter, _, _, _) => capturedFilter = filter)
            .ReturnsAsync(registros);

        var result = await _service.ObtenerAsync(1, 50, "user1");

        result.Should().HaveCount(1);
        result[0].EntidadId.Should().Be("user1");
    }

    [Fact]
    public async Task ObtenerAsync_ConEntidadIdSinResultados_RetornaListaVacia()
    {
        _mockDb.Setup(db => db.FindToListAsync(
                It.IsAny<IMongoCollection<Auditoria>>(),
                It.IsAny<FilterDefinition<Auditoria>>(),
        ...
        var result = await _service.ObtenerAsync(1, 50, "nonexistent");
